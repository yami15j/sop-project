    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#010B2B] font-sans selection:bg-[#00A8E8]/20 selection:text-white px-4 py-4 sm:px-3.5 sm:py-6 md:p-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-[390px] rounded-3xl sm:rounded-[32px] border border-white/10 bg-white px-5 py-6 sm:px-8 sm:py-10 shadow-[0_0_50px_rgba(0,168,232,0.15)] relative z-10">
        ...
        <div className="mb-6 flex justify-start">
          <Link href="/login" className="inline-flex items-center gap-1.5 text-slate-400 hover:text-slate-700 font-bold text-xs transition-colors">
        ...
        {params?.success === '1' ? (
          <div className="flex flex-col items-center text-center animate-in fade-in zoom-in-95 duration-300">
            <div className="w-12 h-12 bg-green-50 rounded-full flex items-center justify-center border border-green-100 mb-4">
              <CheckCircle2 className="w-6 h-6 text-green-500" />
            </div>
            <h3 className="font-extrabold text-slate-800 text-base mb-2">¡Correo Enviado!</h3>
            <p className="text-slate-500 text-xs sm:text-sm leading-relaxed mb-6">
              Hemos enviado instrucciones a <strong className="text-slate-700 font-bold break-all">{params.email}</strong>. Por favor, revisa tu Gmail (incluyendo la carpeta de spam).
            </p>
            <Link
              href="/login"
              className="w-full rounded-xl bg-slate-100 hover:bg-slate-200 px-4 py-3 font-bold text-slate-700 transition-colors text-center text-sm"
            >
